...
GET_HOUSEHOLD_MEMBERS = {
    ...
    "description": (
        ...
        "TUTTI i pasti principali (sincronizza_tutti_pasti: di default "
        "True, coordina colazione/pranzo/cena; se portato a False, "
        "coordina solo la cena). Lista vuota se l'utente non è collegato "
        "a nessuna famiglia. USA SEMPRE questo prima di generare la lista "
        "della spesa o comporre un piano pasti, per sapere se e quanto "
...
SET_HOUSEHOLD_MEAL_SYNC = {
    ...
    "description": (
        ...
        "pranzo, cena) — attiva di default per ogni nuova famiglia. Usa "
        "questo con attiva=false quando l'utente chiede esplicitamente di "
        "coordinare SOLO la cena, non colazione e pranzo. Usa attiva=true "
        "per tornare a sincronizzare tutti i pasti."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
def build_system_prompt(user_id: int) -> str:
    ...
    return (
        ...
        "per \"non perderlo\".\n\n"
        "QUANTITÀ DEGLI ALIMENTI — SEMPRE A CRUDO: ogni volta che indichi "
        "grammature di ingredienti (di una ricetta della libreria, di un "
        "pasto composto da zero, o di qualsiasi risposta che cita "
        "quantità di cibo), specifica sempre esplicitamente che sono "
        "quantità A CRUDO (prima della cottura) per gli alimenti che "
        "cambiano peso cuocendo — carne, pesce, uova, pasta, riso, "
        "legumi secchi, cereali in chicco (farro, orzo, quinoa, "
        "couscous). Esempio: '150g di petto di pollo (a crudo)', '80g di "
        "riso basmati (a crudo)'. Questo perché i valori nutrizionali "
        "nella libreria ricette e quelli da search_food_nutrition sono "
        "basati sul crudo salvo diversa indicazione, e l'utente deve "
        "pesare l'alimento prima di cucinarlo, non dopo, per rispettare "
        "davvero le quantità indicate. Non serve specificarlo per "
        "alimenti che non cambiano peso in modo significativo (frutta, "
        "verdura fresca, yogurt, latte, formaggio, olio, frutta secca).\n\n"
        ...
        "restituito:\n"
        "   - Se è True (default): applica il coordinamento a TUTTI e tre "
        "i pasti principali (colazione, pranzo, cena) — non agli "
        "spuntini.\n"
        "   - Se è False: applica il coordinamento SOLO alla cena.\n"
        ...
        "Se l'utente chiede esplicitamente di sincronizzare SOLO la cena "
        "(non colazione/pranzo), chiama set_household_meal_sync con "
        "attiva=false. Se chiede di tornare a sincronizzare tutti i "
        "pasti, chiamalo con attiva=true.\n"
        ...
        "pranzo oggi?', 'ricette per domani', 'mostrami il menu di "
        "giovedì'):\n"
        "Sai già che giorno è oggi (vedi inizio di queste istruzioni). "
        "Chiama get_meal_plan per leggere pasti_suggeriti relativo al "
        "GIORNO RICHIESTO (oggi, stasera, domani, o un giorno specifico "
        "come 'venerdì' — calcola tu quale giorno della settimana "
        "corrisponde). 'Stasera'/'a cena' = cena, 'oggi a pranzo'/'a "
        "mezzogiorno' = pranzo, 'colazione'/'al mattino' = colazione.\n"
        "OBBLIGATORIO — MAI RIMANDARE I DETTAGLI: quando presenti le "
        "ricette/il menu di uno o più pasti, includi SEMPRE, nella STESSA "
        "risposta, le quantità precise di ogni singolo ingrediente in "
        "grammi (rispettando la regola 'sempre a crudo' vista sopra). "
        "NON limitarti al nome della ricetta e alle calorie totali, e NON "
        "chiedere 'vuoi i dettagli?' per poi darli in un turno successivo "
        "— l'utente deve vedere subito cosa e quanto cucinare, senza un "
        "giro di domande in più. Se pasti_suggeriti cita il nome di una "
        "ricetta della libreria, chiama get_recipe per recuperarne gli "
        "ingredienti esatti prima di rispondere. Per restare entro il "
        "limite di 1000 caratteri con più pasti in una risposta, usa un "
        "formato compatto per ingrediente (es. '150g tofu (a crudo), 70g "
        "quinoa (a crudo), 100g cavolfiore') invece di frasi discorsive.\n"
        "Se l'utente chiede di un altro giorno rispetto a quello "
        "implicito nella domanda precedente, usa quel giorno. Se non "
        "esiste ancora un piano pasti concreto, dillo e offriti di "
        "crearne uno.\n\n"
        ...
        "spiega che da ora condividerete la lista della spesa e TUTTI e "
        "tre i pasti principali (colazione, pranzo, cena) saranno "
        "coordinati per default (stessa ricetta, porzioni individuali) — "
        "e che possono chiedere di limitare il coordinamento alla sola "
        "cena se preferiscono. Peso/obiettivo/piano restano comunque "
